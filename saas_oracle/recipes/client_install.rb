...
template "/app/home/oracle/response/client.rsp" do
  source "client-#{node['saas_oracle']['short_version']}.rsp.erb"
  owner "oracle"
  group "oinstall"
  mode "0644"
  variables({
...
bash "install-oracle" do
  ...
  code <<-EOH
    ...
    trap cleanup EXIT        
    mkdir "$TMPDIR"
    mount -t tmpfs -o size=2G,mode=0755 tmpfs "$TMPDIR"
    curl -s -S -f #{node['saas']['repo']['main']}/common/oracle/#{node['saas_oracle']['version']}/client.tar.gz | tar -C "$TMPDIR" -xz
    sudo -u oracle -i "$TMPDIR"/client/runInstaller -silent -ignorePrereq -waitforcompletion -responseFile /app/home/oracle/response/client.rsp
