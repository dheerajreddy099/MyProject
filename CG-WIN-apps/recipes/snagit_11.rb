...
setup_cwd        = node['CG-WIN-apps']['SnagIt']['script_folder']

dsc_script       = node['CG-WIN-apps']['SnagIt']['dsc_script']
dsc_config       = node['CG-WIN-apps']['SnagIt']['dsc_config']

install_script   = node['CG-WIN-apps']['SnagIt']['install_script']
guard_script     = node['CG-WIN-apps']['SnagIt']['guard_script']

setup_folder     = node['CG-WIN-apps']['SnagIt']['setup_folder']
logs_folder      = node['CG-WIN-apps']['SnagIt']['logs_folder']
# script_log       = "#{logs_folder}\\#{node['CG-WIN-apps']['SnagIt']['software_title']}_script.log"

# Set software installation folder & path variables
software_title   = node['CG-WIN-apps']['SnagIt']['software_title']
software_version = node['CG-WIN-apps']['SnagIt']['software_version']

installer_folder = node['CG-WIN-apps']['SnagIt']['installer_folder']

installer_path   = "#{setup_folder}\\#{installer_folder}"

# Set source archive variables
archive_folder   = node['CG-WIN-apps']['SnagIt']['archive_folder']
archive_file     = node['CG-WIN-apps']['SnagIt']['archive_file']
archive_checksum = node['CG-WIN-apps']['SnagIt']['archive_checksum']
...
# cookbook_file guard_script do
#   path "#{ps_script_cwd}/#{guard_script}"
# end
...
powershell_script install_script do
  cwd "#{setup_cwd}"
  code "#{setup_cwd}\\#{install_script} -title \"#{software_title}\" -version \"#{software_version}\""
  timeout 1200
  action :nothing
